...
public class AutoUpdateService extends Service {
	...
	public static final int UpdateNotificationID=1234;

	private BjVersion mVersion;
	//private NotificationCompat.Builder builder;

	@Override
	public IBinder onBind(Intent intent) {
		return mAutoDownloadServiceBinder;
	}

	private AutoUpdateServiceBinder mAutoDownloadServiceBinder = new AutoUpdateServiceBinder();	
	// 定义内容类继承Binder
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		mVersion = (BjVersion) bundle.getSerializable("version");
		if (true == mVersion.for_release) {
			...
			PendingIntent contentIntent = PendingIntent.getActivity(
					AutoUpdateService.this, 0, new Intent(
							AutoUpdateService.this, LoginActivity.class),
					PendingIntent.FLAG_UPDATE_CURRENT);
			notification.setLatestEventInfo(AutoUpdateService.this,
					getResources().getString(R.string.app_name),
					mVersion.version_changelog, contentIntent);

			// 将下载任务添加到任务栏中
			nm.notify(UpdateNotificationID, notification);

			myHandler = new MyHandler(Looper.myLooper(), AutoUpdateService.this);
			...
			// 启动线程开始执行下载任务
			downFile(mVersion.download_link);

		} else if (true == mVersion.force_update) {
		...
		public void handleMessage(Message msg) {
			...
			if (msg != null) {
				...
				case 2:
					...
					download_precent = 0;
					nm.cancel(UpdateNotificationID);
